...
def imagematting_indexmatting(input_path, trimap, model, output_path):
    ...
    with torch.no_grad():
        image, trimap = np.array(Image.open(input_path)), np.array(Image.open(trimap))
        ...
        Image.fromarray(alpha.astype(np.uint8)).save(output_path)